...
namespace Sort
{
    ...
    public static class ArrayMergeSort
    {
        ...
        public static int[] MergeSort_v2(this int[] array)
        {
            return Sort2(array, 0, array.Length - 1);
        }
        static int[] Sort2(int[] arr, int start, int end)
        {
            int[] result = null;
            ...
            var aLeft = Sort2(arr, start, half);// Сортируем левую часть
            var aRight = Sort2(arr, half + 1, end);// сортируем правую часть
